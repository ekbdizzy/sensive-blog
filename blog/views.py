...
def index(request):
    ...
    most_popular_posts = Post.objects.popular() \
                             .prefetch_related('author')[:5].fetch_with_comments()

    fresh_posts = all_posts.order_by('published_at').fetch_with_comments()
...
def tag_filter(request, tag_title):
    ...
    related_posts = tag.posts.all()[:20].prefetch_related('author').fetch_with_comments()

    most_popular_tags = Tag.objects.popular()[:5]

    most_popular_posts = Post.objects.popular().prefetch_related('author')[:5].fetch_with_comments()
